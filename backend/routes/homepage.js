...
// ดึงชื่อ + สาขาของ user ที่ login อยู่
// ใช้แสดงในหน้า Home: "Hi! Nanno / Faculty : ICT"
...
// __________________ Trip ______________
...
router.get('/recommended', async (req, res) => {
  try {
    ...
    const [[profile]] = await db.query(
      `SELECT tags FROM User_profile WHERE user_id = ?`,
      [userId]
    );
...
// GET /api/trips/my-schedule  (ต้อง login)
// ดึงทริปที่ user กำลังเข้าร่วม เรียงตามวันที่ใกล้ที่สุดก่อน
// router.get('/my-schedule', authRequired, async (req, res) => {
...
router.get('/recent', async (req, res) => {
  try {
    // ดึง reviews พร้อมข้อมูลทริปและคนรีวิว
    const [reviews] = await db.query(`
      SELECT r.*,
             u.user_name,
             up.profile_img AS reviewer_image,
             t.trip_name,
             t.cover_image,
             t.category,
             t.location_name
    ...
    // ดึงสมาชิกที่เข้าร่วมแต่ละทริป (max 10 คนต่อทริป)
